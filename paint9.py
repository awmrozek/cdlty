...
class SimplePaint:
    ...
    def setup_menu(self):
        ...
        self.edit_menu.add_command(label="Paste", command=self.paste_clipboard, accelerator="Ctrl+V")
        ...
        self.root.bind_all("<Control-x>", lambda event: self.cut_selection())
        self.root.bind_all("<Control-c>", lambda event: self.copy_selection())
        self.root.bind_all("<Control-v>", lambda event: self.paste_clipboard())
    ...
    def setup_tools(self):
        ...
        self.text_button = tk.Button(self.tool_frame, text="Text", command=self.use_text)
    ...
    def add_text(self, event):
        ...
        if text:
            ...
            self.draw.text((x, y), text, fill=self.color, font=("Arial", self.brush_size))
            self.unsaved_changes = True

    def select_all(self):
        self.canvas.select_from(0, 0)
        self.canvas.select_to(tk.END)

    def cut_selection(self):
        self.copy_selection()
        self.canvas.delete(tk.SEL_FIRST, tk.SEL_LAST)

    def copy_selection(self):
        self.selected_item = self.canvas.postscript(colormode='color')
        self.selected_area = (self.canvas.bbox(tk.SEL_FIRST), self.canvas.bbox(tk.SEL_LAST))

    def paste_clipboard(self):
        if self.selected_item and self.selected_area:
            x1, y1, x2, y2 = self.selected_area
            width = x2 - x1
            height = y2 - y1
            image = Image.open(io.BytesIO(self.selected_item.encode('utf-8')))
            image = image.resize((width, height), Image.ANTIALIAS)
            self.image.paste(image, (x1, y1))
            self.photo_image = ImageTk.PhotoImage(self.image)
            self.canvas.create_image(0, 0, image=self.photo_image, anchor=tk.NW)
            self.unsaved_changes = True
